...
If you get an error saying something like +`require': cannot load such file -- mkmf+,
...
You might get an error like:
  ...
  /usr/lib/ruby/1.9.1/fileutils.rb:351:in `symlink': File exists - (/.../ServerUtils/addemailaccount.rb, /usr/bin/addemailaccount) (Errno::EEXIST)

This is because the install script doesn't "force write".
So if some of the files (symlinks) that the script is trying to install already exists, you need to remove them first!

There is no need to run the install script if you already have installed SU, if you're updating the scripts (unless there's a new a script).
If there's a new script, the install script will try to "install" them first, so if you get this error on a script that already was installed,
the new script should have been installed!

=== TODO

* Add a script to remove/purge e-mail accounts
* Let the install handle existing files gracefully
